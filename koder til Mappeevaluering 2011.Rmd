...
*Formålet med introduksjonen er å gi leseren en idé om hva den kan forvente seg dersom den leser hele oppgaven (temaet for oppgaven). Introduksjonen skal i tillegg forklare for leseren hvorfor denne skal bruke tid til å lese videre. Introduksjonsdelen kan med fordel holdes relativt kort. Jeg anbefaler at introduksjonen ikke er lengre enn 800 ord (1.5 - 2 sider).*

*For å gi leseren en idé om hva den kan forvente må du starte med å beskrive hva du skal gjøre i oppgaven. For at leseren skal forstå hvorfor den skal lese oppgaven må du motivere hvorfor temaet er interessant å se på. For å gjøre dette må du gi en kort bakgrunn og motivere temaet for oppgaven. Starte bakgrunnen med å definere økonomisk vekst. Diskuter hvorfor det kan være interessant å se på økonomisk vekst og hvilke mulige problemer det finnes med å se på økonomisk vekst som en indikasjon på endringer i velferd.*

*Visualiser gjerne hvordan økonomisk vekst henger sammen med andre mål på velferd, f.eks. forventet levealder. Du kan f eks bruke pakken «gapminder» til dette (du finner eksempler og hjelp her, og her).*
...
data <- gapminder %>% 
  select(country, year, gdpPercap, lifeExp, continent) %>% 
  filter(year == 2007)

plot1 <- ggplot(data, aes(x = gdpPercap, y = lifeExp)) +
  geom_point(aes(size = gdpPercap/1000, color = continent), alpha = 0.5) +
  scale_x_log10(labels = scales::comma) +
  scale_size(range = c(2, 20)) +
  labs(x = "Økonomisk vekst (BNP per innbygger i USD)",
       y = "Forventet levealder (år)",
       title = "Økonomisk vekst og Forventet levealder",
       color = "Kontinent") +
  theme_bw()
...
*Teori-delen skal gjøre rede for teorien som blir testet i den empiriske analysen. I denne oppgaven skal du forklare Solow-modellen for leseren. Mer spesifikkt skal du gjøre forklare hva og hvordan materiell velferd og økonomisk vekst blir bestemt i det lange løp. Forklar i tekst og illustrer grafisk. Vær nøye med å gi økonomisk intuisjon. Det er ikke lov å gjenbruke figurer fra andre kilder. Avslutte teori-seksjonen med å oppsummere prediksjonene fra den teoretiske modellen.*
...
*I avsnitt 3.1 skal du beskrive de data du har brukt for din analyse. I den her oppgaven skal du bruke data fra World Development Indicators, fra Verdens Banken. Datamaterialet er tilgjengelig i Rstudio via pakken «wdi».*

*Du finner en beskrivelse av variablene i kapittel 7 i Hess (2016), og ved å bruke søkefunksjonen i wdi-pakken. Jeg har hentet ut de variabler som dere trenger til analysen og satt sammen et datasett. Jeg beskriver hvordan dere får fram datasettet her nede. Du får gjerne bruke egen kode. Du finner hjelp til variablene som finnes i WDI-pakken her og hvordan du kan lage grafer og analysere data her.*
...
*I resultatdelen skal du analysere hvordan vekstraten i BNP per innbygger korrelerer med følgende variabler: gjennomsnittlig netto-sparing, gjennomsnittlig humankapital (år i skole), gjennomsnittlig uttømning av naturressurser, gjennomsnittlig årlig vekst i investeringer, gjennomsnittlig vekstrate i arbeidskraften og gjennomsnittlig vekstrate i befolkningen, gjennomsnittlig vekstrate i eksport, og BNP per innbygger i år 2000.*
...
*NB: det ser ut å finnes observasjoner i datamaterialet som har en stor innvirkning på resultatet (“outliers” eller “influential observations”). Du kan lese om hvordan vi kan identifisere disse her. I avsnitt 4.2 viser jeg et eksempel på hvordan du kan ta vekk disse observasjonene fra din analyse.Metodedelen avslutter mappeoppgave 1.*


P

### 4. Resultat
*Mappeoppgave 2 starter med resultatdelen. I resultatdelen skal du beskrive resultatet av din empiriske analyse. Du må til stor del selv finne ut hva du skal gjøre for å få fram resultatene.*
...
rownames(table_des)<-c("log BnP per innbygger i 2019","Gjennomsnittlig årlig vekstrate i BNP pc 2000-2019 (%) ","log BNP per innbygger i 2000 ", "Gjennomsnittlig årlig vekstrate i arbeidskraften (%) ","Gjennomsnittlig årlig befolkningsvekst (%) ","Gjennomsnittlig sparing for perioden 2000-2015 (%) ","Gjennomsnittlig årlig vekstrate (negativ) i naturressurser (%) ", "Gjennomsnittlig årlig vekstrate i investeringer (%) ","Gjennomsnittlig årlig vekstrate i eksport (%) ","Gjennomsnittlig antall år i skole ")
...
#Økonometrisk analyse Bruk denne
...
```{r}
df_growth2019_n <- df_growth2019[complete.cases(df_growth2019$region, df_growth2019$poptot, df_growth2019$avg_nsy, df_growth2019$gdppc),] # Tar vekk land som mangler observasjoner. Dette gjør grafen penere.  

plot1 <- ggplot(df_growth2019_n, aes(x = avg_nsy , y = ln_gdppc)) +
  xlab("Gjennomsnittlig sparing for perioden 2000-2015") + # Beskrivelse for x-akselen
  ylab("lnBNP per innbygger") + # Beskrivelse for y-akselen
  theme_classic(base_size = 14, base_family = "Georgia") + # Tekststørrelse og font
  geom_point(aes(size = poptot, color = region), alpha = 0.8) + # Størrelse (farge) på bobblene avhenger befolkningsstørrelse (region)
  geom_smooth(method = "lm", se = FALSE, color = "black") + # Legger til lineær regresjonslinje uten standardfeilområdet
  scale_size_area(guide = "none", max_size = 14) + #Ta vekk legend for befolkningsstørrelse
  theme(legend.text = element_text(size = 10, color = "black"), legend.position = "bottom", legend.title = element_text(size = 8)) +  # Bestem font-størrelse, posisjon og størrelse på legend
  ...
  scale_x_continuous(breaks = c(0, 5, 10, 15, 20, 25, 30, 35)) # Velg hvilke "ticks" som skal vises på x-akselen

plot1 + labs(title = "Forholdet mellom sparing og BNP per innbygger")
